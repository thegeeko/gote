...
const QuestionSlide: React.FC<{ question: Question }> = ({ question }) => {
  return (
    <Link href={`/polls/${question.id}`}>
...
const Home: NextPage = () => {
  ...
  const renderData = () => {
    if (isLoading) return <div className="text-3xl font-bold text-center animate-pulse">Loading ...</div>;

    if (!isLoading && !data) return <div className="text-3xl font-bold text-center text-red-400">Error :(</div>;
    ...
    if (data.length == 0)
      return (
        <div className="text-gray-500 text-4xl text-center font-bold mt-20">
          ...
          <Link href="/new">
            <button className="block mt-10 mx-auto max-w-[185px] px-2 py-2 text-xl bg-green-200 shadow-sm rounded-md w-6/12 md:w-3/12 text-center font-bold border-2 border-black">
              New Poll
            </button>
          </Link>
        </div>
    ...
    return (
      <div className="flex flex-wrap justify-between">
        {data.map((q) => (
          <QuestionSlide question={q} key={q.id} />
        ))}
        <Link href="/new">
          <button className="px-6 py-3 mt-10 text-xl bg-green-200 shadow-sm rounded-md w-6/12 md:w-3/12 text-center font-bold border-4 border-black basis-full">
            New Poll
          </button>
        </Link>
      </div>
